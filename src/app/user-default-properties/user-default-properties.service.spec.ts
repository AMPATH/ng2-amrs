...
describe("User Default Service Unit Tests", () => {
  ...
  const getUrl =
    "https://amrs.ampath.or.ke:8443/amrs/ws/rest/v1/location?v=default";
  ...
  it("should be injected with all dependencies", inject(
    [UserService, LocalStorageService, AppSettingsService],
    (
      user: UserService,
      localStore: LocalStorageService,
      appSettingsServ: AppSettingsService
    ) => {
  ...
  it("should set and get location property", inject(
    [UserDefaultPropertiesMockService, LocalStorageService],
    (
      propertiesResourceService: UserDefaultPropertiesMockService,
      localStore: LocalStorageService
    ) => {
      expect(propertiesResourceService.getCurrentUserDefaultLocation()).toEqual(
        "userDefaultLocationtest"
      );
