...
describe('ShareCohortListComponent', () => {
  ...
  it('should hit the success callback when getCohortUser returns success', (done) => {
  ...
  it('should hit the success callback when createCohortUser returns success', (done) => {
    const spy = spyOn(dataStub, 'createCohortUser').and.returnValue(
      of(expectedPayload)
    );
    comp.getSelectedUser({ value: 'test' });
    spyOn(dataStub, 'getCohortUser').and.returnValue(of(expectedResults));
    comp.getCohortUsers();
