...
export class ShareCohortListComponent implements OnInit, OnDestroy {
  ...
  public getCohortUsers() {
    ...
    const sub = this.cohortUserResourceService.getCohortUser(
      this.selectedCohortUuid
    );
    if (sub) {
  ...
  public ShareCohortWithNewUser() {
    ...
    this.cohortUserResourceService
      .createCohortUser(cohortUserPayload)
      .pipe(take(1))
      .subscribe(
        (success) => {
          if (success) {
            this.displaySuccessAlert(
              "Cohort sharing has been done successfully"
            );
            this.getCohortUsers();
  ...
  public deleteCohortUser() {
    ...
    if (this.selectedCohortUserId) {
      this.cohortUserResourceService
        .voidCohortUser(this.selectedCohortUserId)
        .pipe(take(1))
        .subscribe(
          (success) => {
            if (success) {
  ...
  public updateCohortUserRole(userId, role) {
    ...
    this.cohortUserResourceService
      .updateCohortUser(userId, cohortUserPayload)
      .pipe(take(1))
      .subscribe(
        (success) => {
          if (success) {
