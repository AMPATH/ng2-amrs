...
export class PatientRoutesFactory {
  ...
  public createPatientDashboardRoutes(patient: Patient): RouteModel[] {
    ...
    if (Array.isArray(patientRoutesConfig['programs'])) {
      for (const program of patientRoutesConfig.programs) {
        if (
          program.published &&
          (program.requiresPatientEnrollment === false ||
            this.patientIsInProgram(
              program.programUuid,
              patient.enrolledPrograms
            ))
        ) {
          routes.push(this.createProgramRouteModel(program, patient.uuid));
        }
      }
    }
  ...
  private createProgramRouteModel(
    routInfo: any,
    patientUuid: string
  ): RouteModel {
  ...
  private patientIsInProgram(programUuid: string, patientPrograms): boolean {
    if (Array.isArray(patientPrograms)) {
      for (const program of patientPrograms) {
        if (
          program.programUuid === programUuid &&
          program.dateEnrolled &&
          (program.dateCompleted === null ||
            program.dateCompleted === undefined)
        ) {
          return true;
        }
      }
  ...
  private createProgramChildRoutes(
    routInfo: any[],
    programRouteModel: RouteModel
  ) {
    programRouteModel.childRoutes = [];
    routInfo.forEach((route) => {
      programRouteModel.childRoutes.push(
        this.createProgramChildRoute(route, programRouteModel)
      );
    });
  }

  private createProgramChildRoute(
    routInfo: any,
    programRouteModel: RouteModel
  ): RouteModel {
    const model = new RouteModel();
