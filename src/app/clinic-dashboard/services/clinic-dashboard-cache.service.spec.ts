...
describe("ClinicDashboardCacheService", () => {
  beforeEach(() => {
    TestBed.configureTestingModule({
      declarations: [],
      providers: [ClinicDashboardCacheService, LocalStorageService],
    });
  });
  ...
  it("should be defined", inject(
    ...
    (s: ClinicDashboardCacheService) => {
      expect(s).toBeTruthy();
    }
  ));

  it("should add an item to the cache when add() is called with key and value", inject(
    [ClinicDashboardCacheService],
    (s: ClinicDashboardCacheService) => {
      s.add("test", ["hello"]);
      s.add("test2", "hello");
  ...
  it("should get and item when getByKey() is called with a key", inject(
    [ClinicDashboardCacheService],
    (s: ClinicDashboardCacheService) => {
      s.add("test", ["hello"]);
      s.add("test2", "hello");
  ...
  it("should remove an item when remove() is called with a key", inject(
    [ClinicDashboardCacheService],
    (s: ClinicDashboardCacheService) => {
      s.add("test", ["hello"]);
      s.add("test2", "hello");
  ...
  it("should reset the cache when clear() is called", inject(
    [ClinicDashboardCacheService],
    (s: ClinicDashboardCacheService) => {
      s.add("test", ["hello"]);
      s.add("test2", "hello");
  ...
  it("should have all the required functions defined and callable", inject(
    [ClinicDashboardCacheService],
    (s: ClinicDashboardCacheService) => {
