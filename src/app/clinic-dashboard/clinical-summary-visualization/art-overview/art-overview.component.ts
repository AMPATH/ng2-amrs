            ...
            events: {
              click: function () {
                ...
                if (that.options && that.options.filters.endDate) {
                  that.goToPatientList(indicators[this.name], that.options.filters);
                }
  ...
  processChartData() {
    ...
    for (let indicator in data) {
      if (!indicator.match(new RegExp('location_uuid|location_id|patients'))) {
        this.categories.push(this.clinicalSummaryVisualizationService.translateColumns[indicator]);
        this.series.push({
          name: this.clinicalSummaryVisualizationService.translateColumns[indicator],
          y: data[indicator],
          color: colors[i]
        });
        i++;
