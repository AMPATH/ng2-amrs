...
export class ClinicLabOrdersComponent implements OnInit, OnDestroy {
  ...
  public ngOnInit() {
    ...
    this.gridOptions.getRowHeight = (params) => {
      return (
        20 *
        (Math.floor(params.data.identifiers.length / 15) + 1 &&
  ...
  public downloadPdf(): void {
    ...
    const docDefinition = {
      ...
      footer: (page, pages) => {
        return {
          text:
            'Lab orders report generated for ' +
            this.locationName +
            ' From: ' +
            this.startDate +
            ' To: ' +
  ...
  private getRptBodyData(headers) {
    ...
    _.each(this.results, (data: any) => {
      const dataRow = [];
      const sampleDrawn = data.sample_drawn
        ? data.sample_drawn
      ...
      dataRow.push({
        text: data.identifiers.replace(/,/g, ', '),
        style: 'cellData'
      });
    ...
    for (let i = 0; i < result.length; ++i) {
      ...
      for (const r in data) {
        if (data.hasOwnProperty(r)) {
          const dateActivated = Moment(data.date_activated).format(
            'DD-MM-YYYY'
          );
  ...
  private createColumnDefs() {
    return [
      ...
      },
      {
        ...
        cellRenderer: (params) => {
          if (
            params.data.sample_drawn === null ||
            params.data.sample_drawn === 'NO'
          ) {
            return ' ';
          }
          return params.value;
