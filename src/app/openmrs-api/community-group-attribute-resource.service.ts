...
export class CommunityGroupAttributeService {
  ...
  public getOpenMrsBaseUrl(): string {
    return (
      this._appSettingsService.getOpenmrsRestbaseurl() +
      'cohortm/cohortattribute/'
    );
  }

  public saveGroupAttribute(
  ...
  public getAttributesByAttributeType(attributeTypeName: string) {
    const url = this.getOpenMrsBaseUrl();
    const params = new HttpParams().set('attributeType', attributeTypeName);
    return this.http
