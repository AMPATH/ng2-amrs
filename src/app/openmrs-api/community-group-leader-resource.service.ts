...
export class CommunityGroupLeaderService {
  ...
  public getOpenMrsBaseUrl(): string {
    return (
      this._appSettingsService.getOpenmrsRestbaseurl() + "cohortm/cohortleader/"
    );
  }

  public getGroupLeaderByGroupUuid(GroupUuid: string): Observable<any> {
    const params = new HttpParams().set("v", this.v).set("Group", GroupUuid);
  ...
  public updateGroupLeader(
    groupUuid: string,
    oldLeaderUuid: string,
    newLeaderUuid: string
  ): Observable<any> {
    return this.removeGroupLeader(oldLeaderUuid).flatMap((res) =>
      this.addGroupLeader(groupUuid, newLeaderUuid, new Date())
    );
  }

  public addGroupLeader(
    groupUuid: string,
    personUuid: string,
    startDate: Date
  ): Observable<any> {
