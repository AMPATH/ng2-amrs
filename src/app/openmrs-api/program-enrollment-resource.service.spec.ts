...
describe('Service: ProgramEnrollmentResourceService', () => {
  ...
  it('should call the right endpoint when saving the patient enrollment state', async(() => {
    ...
    expect(req.request.url).toBe(service.getUrl() + '/uuid-1');
  ...
  it('should call the right endpoint when updating the patient enrollment state', async(() => {
    ...
    service
      .updateProgramEnrollmentState(programEnrollmentUuid, payload)
      .subscribe();

    const req = httpMock.expectOne(
      service.getUrl() + '/' + programEnrollmentUuid + '/' + 'state' + '/uuid-1'
    );
    expect(req.request.method).toBe('POST');
    expect(req.request.url).toBe(
      service.getUrl() + '/programenrollment-uuid/state/uuid-1'
    );
    req.flush(JSON.stringify(programEnrollmentResponse));
  }));
