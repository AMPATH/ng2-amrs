...
describe("Service : CohortMemberResourceService Unit Tests", () => {
    ...
    it("should hit right endpoint for getallCohortMembers and get right response", async(() => {
      ...
      const request = httpMock.expectOne(
        (req) =>
          req.method === "GET" &&
          req.url ===
            cohortMemberService.baseOpenMrsUrl +
              "cohort/" +
              parentUuid +
              "/member"
      );
      expect(request.request.url).toEqual(
        cohortMemberService.baseOpenMrsUrl + "cohort/" + parentUuid + "/member"
      );
      expect(request.request.method).toBe("GET");
      expect(request.request.url).toContain(
        "/ws/rest/v1/cohort/" + parentUuid + "/member"
      );
  ...
  describe("Add Cohort Member", () => {
    ...
    it("should hit right endpoint for add Cohort and get right response", () => {
      cohortMemberService
        .addCohortMember(parentUuid, addCohortMemberPayload)
        .subscribe((res) => {
      ...
      expect(postRequest.request.url).toContain(
        "cohort/" + parentUuid + "/member"
      );
  ...
  describe("Retire Cohort Member", () => {
    ...
    it("should hit right endpoint for Retire Cohort Member and get right response", () => {
      cohortMemberService
        .retireCohortMember(parentUuid, uuid)
        .subscribe((res) => {
