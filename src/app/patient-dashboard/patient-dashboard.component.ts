...
export class PatientDashboardComponent implements OnInit, OnDestroy, DoCheck {
  ...
  public getNewResults() {
    const sub1 = this.patientService.currentlyLoadedPatient.subscribe(
      (patient: any) => {
        if (patient) {
          const sub2 = this.getCombinedResult(patient)
            .pipe(take(1))
            .subscribe(
              (results: any[]) => {
                    ...
                    for (const test of uniqueTests) {
                      if (test.groupMembers) {
                        for (const l of test.groupMembers) {
                          if (
                            l.uuid === '5538cd04-9852-40f8-88ba-c69da32e50eb'
                          ) {
                            content = content + `CD4%: ${l.value} `;
                          }
                          if (
                            l.uuid === 'f9424af5-1fd3-4a8f-8d43-7b098eb20ac3'
                          ) {
                            content = content + `CD4: ${l.value} `;
                          }
                        }
                        ...
                        content =
                          content +
                          `${test.display} (collected on ${Moment(
                            test.obsDatetime
                          ).format('DD/MM/YYYY')})`;
                      }
                    }
