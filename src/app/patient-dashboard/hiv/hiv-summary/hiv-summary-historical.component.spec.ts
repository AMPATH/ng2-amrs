...
describe('Component: HivSummaryHistorical Unit Tests', () => {
  ...
  beforeEach(() => {
    ...
    component = new HivSummaryHistoricalComponent(
      hivSummaryService,
      patientService
    );
  });
  ...
  it('should return true of column exists and has a value', (done) => {
    ...
    expect(component.hasColumnData(testHasColumn, 'med_pickup_rtc_date')).toBe(
      true
    );
    expect(component.hasColumnData(testNoCol, 'med_pickup_rtc_date')).toBe(
      false
    );
    expect(
      component.hasColumnData(testHasNullColValue, 'med_pickup_rtc_date')
    ).toBe(false);
