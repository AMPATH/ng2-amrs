        ...
        map((data) => {
          if (data) {
            ...
            _.each(allAvailablePrograms, (program: any) => {
              ...
              const _enrolledPrograms = _.filter(
                enrolledPrograms,
                (enrolledProgram: any) =>
                  enrolledProgram.programUuid === program.uuid &&
                  !enrolledProgram.voided
              );

              const route: any = _.find(
                dashboardRoutesConfig.programs,
                (_route: any) =>
                  _route['requiresPatientEnrollment'] &&
                  _route['programUuid'] === program.uuid
              );
              ...
              _programs.push({
                ...
                dateCompleted:
                  ...
                  !_.isNil(_enrolledProgram.dateCompleted)
                    ? this._datePipe.transform(
                        _enrolledProgram.dateCompleted,
                        'yyyy-MM-dd'
                      )
                    : null,
                validationError: '',
