...
export class LandingPageComponent implements OnInit, OnDestroy {
  ...
  public editPatientEnrollment(row: any) {
    console.log('this.isEditLocation', this.isEditLocation);
    row.isFocused = true;
    this.isEdit = true;
    let payload = {};
  ...
  private isValidForm(row: any) {
    let location = '';
    if (row.enrolledProgram.openmrsModel.location) {
      location = row.enrolledProgram.openmrsModel.location.uuid;
    }
    if (!this._formFieldsValid(row.dateEnrolled, row.dateCompleted, location
        )) {
