...
describe('Component: TodaysVitalsComponent', () => {
  ...
  it("should display today's vitals after the component initializes", async(() => {
    ...
    const vitalsList = <HTMLElement>(
      nativeElement.querySelector('li.list-group-item')
    );
    expect(vitalsList.innerHTML).toMatch(
      /No vitals taken for the patient today/
    );
    ...
    fixture.whenStable().then(() => {
      ...
      expect(vitalValues[0].innerHTML).toMatch(
        /123/,
        'Blood pressure: Systolic'
      );
      const compoundVitalValue = <HTMLElement>(
        nativeElement.querySelector('.compoundValue')
      );
      expect(compoundVitalValue.innerHTML).toMatch(
        /50/,
  ...
  it('should display a different set of vitals specific to the patient when a new patient is loaded', async(() => {
  ...
  it("should display an error if fetching the current day's vitals fails", async(() => {
    spyOn(vitalsService, 'getTodaysVitals').and.returnValue(
      new Promise((resolve, reject) =>
        reject(new Error('Could not get vitals'))
      )
    );
    fixture.detectChanges();
    ...
    fixture.whenStable().then(() => {
      fixture.detectChanges();
      const errorDiv = <HTMLElement>(
        nativeElement.querySelector('div.alert.alert-danger.alert-dismissible')
      );
      expect(errorDiv.innerHTML).toContain("Error fetching today's vitals");
      const errorButton = <HTMLButtonElement>(
        nativeElement.querySelector('button.close')
      );
  ...
  it("should fetch and return the current day's encounters", () => {
    const todaysEncounters = component.getTodaysEncounters(
      testPatient1.encounters
    );
    expect(todaysEncounters.length).toBeGreaterThan(0);
    expect(todaysEncounters[0].encounterType.display).toEqual(
      'ANTICOAGULATION TRIAGE'
    );
    expect(todaysEncounters[0].encounterType.uuid).toEqual(
      '6accd920-6254-4063-bfd1-0e1b70b3f201'
    );
    expect(todaysEncounters[0].form.name).toEqual(
      'ONCOLOGY POC Anticoagulation Triage Form'
    );
    expect(todaysEncounters[0].form.uuid).toEqual(
      '84539fd3-842c-46a7-a595-fc64919badd6'
    );
    expect(todaysEncounters[0].location.display).toEqual('Location Test');
    expect(todaysEncounters[0].location.uuid).toEqual(
      '18c343eb-b353-462a-9139-b16606e6b6c2'
    );
    expect(todaysEncounters[0].patient.uuid).toEqual(
      '7ce98cb8-9785-4467-91cc-64afa2d59763'
    );
  });
  ...
  it("should fetch and return the details of the currrent day's encounter", () => {
    ...
    todaysEncounterDetails.then(
      (encounterDetails) => {
        ...
        expect(encounterDetails[0].encounterType.uuid).toEqual(
          '6accd920-6254-4063-bfd1-0e1b70b3f201'
        );
        expect(encounterDetails[0].form.name).toEqual(
          'ONCOLOGY POC Anticoagulation Triage Form'
        );
        expect(encounterDetails[0].form.uuid).toEqual(
          '84539fd3-842c-46a7-a595-fc64919badd6'
        );
        expect(encounterDetails[0].location.display).toEqual('Location Test');
        expect(encounterDetails[0].location.uuid).toEqual(
          '18c343eb-b353-462a-9139-b16606e6b6c2'
        );
        expect(encounterDetails[0].patient.uuid).toEqual(
          '7ce98cb8-9785-4467-91cc-64afa2d59763'
        );
      },
      (err) => {
