...
export class FormListService {
  ...
  public sortFormList(unsortArray, sortingMetadataArrays) {
    ...
    _.each(sortingMetadataArrays, (array) => {
      if (!Array.isArray(array)) {
        throw new Error(
          'Every member of the sortingMetadataArrays  must be an array'
        );
      }
    });
  ...
  public removeVersionInformation(formName) {
    ...
    if (
      this._isVersionInformation(
        lastFiveCharacters.substr(
          indexOfV,
          lastFiveCharacters.length - indexOfV
        )
      )
    ) {
  ...
  public getFormList() {
    ...
    this.formsResourceService.getForms().subscribe((forms) => {
      this.formOrderMetaDataService
        .getDefaultFormOrder()
        .subscribe((defaultOrder) => {
          const formlist = this.processFavouriteForms(
            this._getFormList(forms, [favouriteForms, defaultOrder]),
            favouriteForms
          );
