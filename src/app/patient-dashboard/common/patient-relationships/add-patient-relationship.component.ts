...
export class AddPatientRelationshipComponent implements OnInit, AfterViewInit {
  ...
  public ngOnInit(): void {
    ...
    this.appFeatureAnalytics.trackEvent(
      'Patient Dashboard',
      'Add Patient Relationship Loaded',
      'ngOnInit'
    );
  }
  ...
  public saveRelationship() {
    if (
      ...
      this.patientRelationshipService
        .saveRelationship(patientRelationshipPayload)
        .pipe(take(1))
        .subscribe(
          (success) => {
            if (success) {
          ...
          (error) => {
            ...
            this.displayErrorAlert(
              'Error!',
              'The system encountered an' + ' error while saving relationship'
            );
          }
        );
    }
    ...
    if (
      ...
      this.displayErrorAlert(
        'Validation Error!',
        'Please make sure you fill the necessary' + ' fields before submiting'
      );
    }
  }
