...
@Component({
  ...
  template: `
    <button
      *ngIf="isDraftFormPresent"
      (click)="loadDraftedForm()"
      class="btn btn-warning "
      style="float: right;margin-right:4px; margin-top:4px;"
    >
...
export class DraftedFormNavComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.subscription =
      this.patientService.currentlyLoadedPatientUuid.subscribe(
        (uuid) => (this.patientUuid = uuid)
      );
  }
  ...
  public loadDraftedForm() {
    ...
    const url = ['/patient-dashboard', 'patient', this.patientUuid].concat(
      urlPaths
    );
