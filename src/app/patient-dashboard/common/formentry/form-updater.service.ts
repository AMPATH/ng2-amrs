...
export class FormUpdaterService {
  ...
  private doesUpdatedSchemaExist(uuid, cachedSchema): Observable<any> {
    ...
    private checkUpdatedForms() {
      ...
      return this.formsResourceService.getForms()
      .switchMap((forms: any[]) => {
        ...
        if (arrayOfObservables.length > 0) {
          ...
          return Observable.from(arrayOfObservables).combineAll(); } else {
  ...
  private showToastWithSpinner(message) {
    this.toast.openFromComponent(ToastComponent, { data: message, extraClasses: ['snackbar']});
  }

  private showPlainToast(message: string, duration?: number) {
    if (duration) {
      this.toast.open(message, '', {duration: duration, extraClasses: ['snackbar'] });
    } else {
