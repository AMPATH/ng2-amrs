...
export class PrettyEncounterViewerComponent implements OnInit {
  ...
  public wireDataSources() {
    ...
    this.dataSources.registerDataSource('patientInfo', {
      ...
      mhn: this.patient.openmrsModel.identifiers.find(
        (identifier) =>
          identifier.identifierType.name === 'MTRH Hospital Number'
      )
        ? // tslint:disable-next-line:max-line-length
          this.patient.openmrsModel.identifiers.find(
            (identifier) =>
              identifier.identifierType.name === 'MTRH Hospital Number'
          ).identifier
        : ''
  ...
  public displayEncounterObs(encounter) {
    ...
    this.encounterResourceService
        ...
        flatMap((encounterWithObs) => {
          ...
          if (encounterWithObs.form) {
            if (this.isPOCForm(encounterWithObs.form)) {
              return this.formSchemaService.getFormSchemaByUuid(
                encounter.form.uuid
              );
            } else {
              this
                .showErrorMessage(`This encounter was done using an Infopath form.
      ...
      .subscribe((compiledSchema) => {
        const unpopulatedform = this.formFactory.createForm(
          compiledSchema,
          this.dataSources
        );
        this.encounterAdapter.populateForm(
          unpopulatedform,
