...
describe('Service: PatientService', () => {
  ...
  it(
    'should initialized currently loaded patient observable when Patient service is injected ' +
      'for the first time',
    () => {
      const service: PatientService = TestBed.get(PatientService);
  ...
  it(
    'should not hit the server when setCurrentlyLoadedPatientByUuid is called' +
      ' with a uuid of an existing loaded patient of same uuid',
    inject(
      [PatientResourceService],
      fakeAsync((patientResourceService: PatientResourceService) => {
        const service: PatientService = TestBed.get(PatientService);
        const uuid = 'patient-uuid-1';
        const patientObject: Patient = new Patient({
          uuid: uuid,
          encounters: []
        });
  ...
  it('should fetch patient object from server if currentlyLoadedPatient uuid is not set', inject(
    [PatientResourceService],
    fakeAsync((patientResourceService: PatientResourceService) => {
      const service: PatientService = TestBed.get(PatientService);
      const uuid = 'patient-uuid-1';
      spyOn(patientResourceService, 'getPatientByUuid').and.callFake(
        (params) => {
  ...
  it(
    'should fetch patient object afresh from server if currentlyLoadedPatient uuid is not  ' +
      'the same as uuid supplied to setCurrentlyLoadedPatientByUuid',
    inject(
      [PatientResourceService],
      fakeAsync((patientResourceService: PatientResourceService) => {
        const service: PatientService = TestBed.get(PatientService);
  ...
  it('should notify all subscribers when patient object changes', (done) => {
    ...
    patientService.currentlyLoadedPatient.subscribe((patient) => {
  ...
  it('should notify all subscribers when current patient uuid changes', (done) => {
    ...
    patientService.currentlyLoadedPatientUuid.subscribe((uuid) => {
      if (uuid === 'init uuid') {
        console.log('got notification for the uuid: init uuid');
      }
  ...
  it(
    'should fetch patient program enrollment information when setCurrentlyLoadedPatientByUuid' +
      ' is called',
    inject(
      [PatientResourceService, PatientProgramService, EncounterResourceService],
      fakeAsync(
        (
          ...
          spyOn(
            patientProgramService,
            'getCurrentlyEnrolledPatientPrograms'
          ).and.callFake((params) => {
            const subject = new BehaviorSubject<any>({});
            subject.next([
              {
          ...
          tick(1000);
          expect(
            patientProgramService.getCurrentlyEnrolledPatientPrograms
          ).toHaveBeenCalled();
        }
      )
    )
  ...
  it(
    'should fetch patient object when fetchPatientByPatientUuid is called with a' +
      ' valid patient uuid',
    inject(
