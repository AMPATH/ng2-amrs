...
describe('HivSummaryService Unit Tests', () => {
  ...
  it('should be injected with all dependencies', inject(
    [HivSummaryResourceService],
    (hivSummaryResourceService: HivSummaryResourceService) =>
  ...
  it('should make API call with the correct url parameters', () => {
    ...
    service.getHivSummary(patientUuid, startIndex, limit).subscribe((data) => {
  ...
  it('should throw an error when server returns an error response', () => {
    ...
    service.getHivSummary(patientUuid, startIndex, limit).subscribe(
      (response) => {
