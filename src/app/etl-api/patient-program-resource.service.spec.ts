...
describe('Patient Program Resource Service Unit Tests', () => {
  ...
  it('should make API call to get all program visit configs', (done) => {
    const appsetting = TestBed.get(AppSettingsService);
    service.getAllProgramVisitConfigs().subscribe((response) => {
      done();
    });

    const req = httpMock.expectOne(
      appsetting.getEtlRestbaseurl().trim() + 'program-visit-configs'
    );
    expect(req.request.method).toBe('GET');
  ...
  it('should make API call to get patient program visit configs', (done) => {
    const appsetting = TestBed.get(AppSettingsService);
    service.getPatientProgramVisitConfigs('uuid').subscribe((response) => {
      done();
    });
    const req = httpMock.expectOne(
      appsetting.getEtlRestbaseurl().trim() +
  ...
  it('should return an error when fetching all program configs fail', async(() => {
    service.getAllProgramVisitConfigs().subscribe(
      (data) => {},
      (error: Error) => {
