...
describe('LabsResourceService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [LabsResourceService, LocalStorageService, AppSettingsService]
    });
  ...
  xdescribe('get New Lab Data By PatientUuid', () => {
    const params = {
    ...
    it('should call the right endpoint', async(() => {
      ...
      const result = service
        .getNewPatientLabResults(params)
        .subscribe((results) => {});
      const req = httpMock.expectOne(
        service.getUrl() +
          '?startDate=2014-12-01&endDate=2014-12-02&patientUuId=uuid'
      );
      ...
      expect(req.request.urlWithParams).toContain(
        `${appSettingsService.getEtlRestbaseurl().trim()}patient-lab-orders`
      );
    ...
    it('should parse response from patient labs sync endpoint', async(() => {
      ...
      result.subscribe((res) => {
        ...
        expect(res[0].concept.uuid).toBe(
          newLabResults.updatedObs[0].concept.uuid
        );
      });
      const req = httpMock.expectOne(
        service.getUrl() +
  ...
  describe('get Historical Lab Data By PatientUuid', () => {
    ...
    it('should call the right endpoint', async(() => {
      ...
      const result = service
        .getHistoricalPatientLabResults(patientUuId, params)
        .subscribe((results) => {});
      const req = httpMock.expectOne(
        appSettingsService.getEtlRestbaseurl().trim() +
          `patient/${patientUuId}/data?startIndex=0&limit=20`
      );
      expect(req.request.urlWithParams).toContain(
        `${appSettingsService
          .getEtlRestbaseurl()
          .trim()}patient/${patientUuId}/data`
      );
      expect(req.request.urlWithParams).toContain(
    ...
    it('should set startIndex to 0 when startIndex is not provided', async(() => {
    ...
    it('should set limit to 20 when startIndex is not provided', async(() => {
      const appSettingsService = TestBed.get(AppSettingsService);
      delete params.limit;
      const result = service
        .getHistoricalPatientLabResults(patientUuId, params)
    ...
    it('should parse response from patient labs  endpoint', async(() => {
      ...
      const result = service.getHistoricalPatientLabResults(
        patientUuId,
        params
      );
