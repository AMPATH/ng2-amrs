...
export class ProgramEnrollmentPatientListComponent
  ...
  public ngOnInit() {
    ...
    if (userDefaultDepartment[0].itemName === 'HIV') {
      ...
      this.enrollmentColdef = _.concat(
        this.defaultEnrollmentColdef,
        hivColumns as Array<object>
      );
    } else {
  ...
  public getEnrolledPatientList(params) {
    ...
    if (typeof params !== 'undefined') {
      this._patientProgramEnrollmentService
        .getActivePatientEnrollmentPatientList(params)
        .pipe(take(1))
  ...
  public processEnrollments(enrollments: any) {
    ...
    _.each(enrollments, (enrollment: any) => {
      ...
      const enrollmentDateDetail =
        enrollment.program_name +
        '( Enrolled - ' +
        Moment(enrollment.date_enrolled).format('DD-MMM-YYYY') +
        ')' +
        completedDetail;
      ...
      if (typeof patientObjMap === 'undefined') {
        ...
        patientObjMap.program =
          patientObjMap.program + ' </br>' + enrollmentDateDetail;
