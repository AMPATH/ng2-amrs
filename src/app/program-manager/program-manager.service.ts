...
export class ProgramManagerService {
  ...
  .subscribe((programs) => {
    ...
    // current program and previous program
    // get previous and current program and send on tye payload
  ...
  private getLastProgramEnrolled(programs) {
    ...
    _.each(programs, item => {
        ...
        if (new Date(d).getTime() === latestDate) {
           lastProgramUuid = item.program.uuid;
        }
    });
    return lastProgramUuid;
  }

  private extendProgramInfo(programInfo, lastEnrolledProgram) {
    return  _.extend(programInfo, {
        patientProgramIdCurrent: programInfo.programUuid,
