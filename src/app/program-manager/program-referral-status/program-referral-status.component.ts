...
export class ProgramReferralStatusComponent implements OnInit {
  ...
  public isValidReferral: boolean;
  private _status: any;
          ...
          if (reply) {
            ...
            _.each(replyWithoutEmpties, (referral: any) => {
              existingReferral = _.find(enrolledPrograms, (program: any) => {
                ...
                // return program.enrolledProgram.uuid === referral.patient_program_uuid;
              });
              return;
            });
            // check that we're not referring to the same program
            // if (existingReferral && !_.isNil(existingReferral.notification_status) && (existingReferral.programUuid !== programUuid)) {
            if (existingReferral && _.isNil(existingReferral.notification_status)
              ...
              // referral notification_status is 1 which means the referral is complete
