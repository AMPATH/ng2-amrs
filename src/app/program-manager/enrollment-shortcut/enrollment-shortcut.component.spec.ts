...
const testDeptProgramsConfig = {
  uud1: {
    ...
    programs: [
      ...
      {
        uuid: '9c64af03-f712-411e-8880-16e98dcdb4a6',
        name: 'HEI_MODEL'
      },
      {
        uuid: '20838ff5-7a28-4877-889c-300155627a6f',
        name: 'FAST_TRACK_FACILITY_CARE_MODEL'
      },

      {
        uuid: '379038fc-663f-42ed-87f3-9cdde7fb4339',
        name: 'PEER_LED_FACILITY_ART_GROUP_MODEL'
      },
      {
        uuid: '	fb36b5af-3f83-460b-a10f-fc7923ed7914',
        name: 'HCW_FACILITY_ART_DISTRIBUTION_MODEL'
      },
      {
        uuid: '29a9df4d-808f-4ba0-8b1e-ea05c918f14b',
        name: 'MULTI_MONTH_DISPENSING_MODEL'
      },
      {
        uuid: '	771b200c-8525-4425-b763-7e1cdca1b01f',
      ...
      {
        uuid: '3ec28e73-6f84-46f1-9310-c0c1a21c8ec3',
        name: 'INDIVIDUAL_PATIENT_ART_DISTRIBUTION_MODEL'
      },
      {
        uuid: 'd9108db3-1cb4-4641-afd7-04f3dfc6a204',
        name: 'COMMUNITY_PHARMACY_MODEL'
      },
      {
        uuid: '5af988e4-09d8-41a4-9438-30f2b62d90b8',
        name: 'FAMILY_COMMUNITY_ART_GROUP_MODEL'
      },
      {
        uuid: 'ce562f55-bf51-4d00-9a2a-f56ca1a8bc34',
  ...
  uud2: {
    ...
    programs: [
      ...
      {
        uuid: '43b42170-b3ce-4e03-9390-6bd78384ac06',
        name: 'GYN-ONCOLOGY TREATMENT PROGRAM'
      },
      {
        uuid: '88566621-828f-4569-9af5-c54f8237750a',
        name: 'BREAST CANCER TREATMENT PROGRAM'
      },
      {
        uuid: 'e48b266e-4d80-41f8-a56a-a8ce5449ebc6',
        name: 'SICKLE CELL PROGRAM'
      },
      {
        uuid: '698b7153-bff3-4931-9638-d279ca47b32e',
        name: 'MULTIPLE MYELOMA PROGRAM'
      },
      {
        uuid: 'a3610ba4-9811-46b3-9628-83ec9310be13',
      ...
      {
        uuid: 'e8bc5036-1462-44fa-bcfe-ced21eae2790',
        name: 'LUNG CANCER TREATMENT PROGRAM'
      },
      {
        uuid: '418fe011-a903-4862-93d4-5e7c84d9c253',
...
describe('Component', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [EnrollmentShortcutComponent],
      providers: [
        AppSettingsService,
        CacheService,
        DataCacheService,
  ...
  beforeEach(() => {
    ...
    localStorageService.getItem.and.returnValue(
      JSON.stringify([{ itemName: 'HEMATO-ONCOLOGY', id: 'uud2' }])
    );
  });
  ...
  it('should fetch and display all the compatible programs the patient can enroll in after the component initializes', () => {
    ...
    expect(component.patient.display).toEqual(
      '749138740-8 - Test Male Treatment'
    );
    expect(component.patientEnrolledPrograms.length).toEqual(
      1,
      'One enrollment'
    );
    expect(component.patientEnrolledPrograms).toEqual(
      testPatient.enrolledPrograms
    );
    expect(component.allDepartmentsProgramsConf.length).toEqual(
      2,
      'HEMATO-ONCOLOGY and HIV program configs'
    );
    expect(component.allProgramsInDefaultDepartmentConf.length).toEqual(
      10,
      '10 programs'
    );
    // Gender incompatible programs removed
    expect(component.patientEnrollablePrograms).not.toContain(
      'CERVICAL CANCER SCREENING PROGRAM'
    );
    expect(component.patientEnrollablePrograms).not.toContain(
      'GYN-ONCOLOGY TREATMENT PROGRAM'
    );
    expect(component.patientEnrollablePrograms.length).toEqual(
      8,
    ...
    const programSelectBtn = <HTMLButtonElement>(
      nativeEl.querySelector('.btn.btn-primary.btn-sm.dropdown-toggle')
    );
    expect(programSelectBtn.innerHTML).toMatch(/Select Program to Enroll/);
    expect(programsList[0].innerText).toEqual(
      'BREAST CANCER SCREENING PROGRAM'
    );
    expect(programsList[1].innerText).toEqual(
    ...
    expect(programsList[7].innerText).toEqual(
      'ANTICOAGULATION TREATMENT PROGRAM'
    );
  });
