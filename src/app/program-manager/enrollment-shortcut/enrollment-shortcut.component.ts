...
export class EnrollmentShortcutComponent implements OnInit {
  ...
  public subscribeToPatientChanges() {
    this.patientSub = this.patientService.currentlyLoadedPatient.subscribe(
      (patient) => {
        if (patient) {
  ...
  public determineProgramsPatientEnrolledIn() {
    ...
    if (this.patient !== null) {
      this.patientEnrolledPrograms = _.filter(
  ...
  public fetchAllProgramsAndDepartments() {
    ...
    this.departmentProgramService
      ...
      .subscribe(
        (results) => {
          if (results) {
            this.allDepartmentsProgramsConf = _.orderBy(
              results,
              ["name"],
              ["asc"]
            );
  ...
  public filterProgramsByDefaultDepartment() {
    ...
    if (
      this.defaultDepartment !== null &&
      this.allDepartmentsProgramsConf !== null
    ) {
  ...
  public determinePossibleProgramsForPatient() {
    ...
    if (
      ...
      const availablePrograms = _.filter(
        this.allProgramsInDefaultDepartmentConf,
        (program) => {
          const enrolledProgUuids = _.map(
