...
import { JsExpressionHelper } from '@ampath-kenya/ngx-openmrs-formentry';
...
export class ZscoreService {
  ...
  public getZRefByGenderAndAge(gender, birthDate, refdate) {
    ...
    if (gender === 'F') {
      if (age < 5) {
        ...
        scoreRefModel.heightForAgeRef = this.getScoreReference(
          hfaFemaleBelow5,
          'Day',
          ageInDays
        );
      ...
      if (age >= 5 && age < 18) {
        scoreRefModel.bmiForAgeRef = this.getScoreReference(
          bfaFemale5Above,
          'Month',
          ageInMonths
        );
  ...
  public getZScoreByGenderAndAge(gender, birthDate, refdate, height, weight) {
    ...
    scoreModel.bmiForAge = helper.calcBMIForAgeZscore(
      refModel.bmiForAgeRef,
      height,
      weight
    );
    scoreModel.weightForHeight = helper.calcWeightForHeightZscore(
      refModel.weightForHeightRef,
      height,
      weight
    );
    scoreModel.heightForAge = helper.calcHeightForAgeZscore(
      refModel.heightForAgeRef,
      height,
      weight
    );
    return scoreModel;
