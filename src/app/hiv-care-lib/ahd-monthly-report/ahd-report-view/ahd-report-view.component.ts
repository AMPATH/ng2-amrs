...
@Component({
  ...
  templateUrl: './ahd-report-view.component.html',
  styleUrls: ['./ahd-report-view.component.css']
})
export class AhdReportViewComponent implements OnInit {
  @Input() SummaryData = [];
  ...
  public setColumns(sectionsData: Array<any>) {
    ...
    for (let i = 0; i < sectionsData.length; i++) {
      ...
      for (let j = 0; j < section.indicators.length; j++) {
        ...
        let u15_f_indicatorDefinition,
          u15_m_indicatorDefinition,
          o15_f_indicatorDefinition,
          o15_m_indicatorDefinition,
          totalDefinition,
          percentageDefinition;

        if (isArrayIndicator) {
          u15_f_indicatorDefinition = indicator.indicator[0];
          u15_m_indicatorDefinition = indicator.indicator[1];
          o15_f_indicatorDefinition = indicator.indicator[2];
          o15_m_indicatorDefinition = indicator.indicator[3];
          totalDefinition = indicator.indicator[4];
          percentageDefinition = indicator.indicator[5];
        } else {
          totalDefinition = indicator.indicator;
        }

        const child: any = {
          headerName: indicator.label,
          refno: indicator.refno,
        ...
        this.sectionIndicatorsValues.forEach((element) => {
          ...
          if (isArrayIndicator) {
            if (
              (element[u15_m_indicatorDefinition] ||
                element[u15_m_indicatorDefinition] === 0) &&
              (element[u15_f_indicatorDefinition] ||
                element[u15_f_indicatorDefinition] === 0) &&
              (element[o15_m_indicatorDefinition] ||
                element[o15_m_indicatorDefinition] === 0) &&
              (element[o15_f_indicatorDefinition] ||
                element[o15_f_indicatorDefinition] === 0)
            ) {
              val.value[0] = element[u15_f_indicatorDefinition];
              val.value[1] = element[u15_m_indicatorDefinition];
              val.value[2] = element[o15_f_indicatorDefinition];
              val.value[3] = element[o15_m_indicatorDefinition];
              // val.value[4] = element[totalDefinition];
              // val.value[5] = element[percentageDefinition];

              sumOfValue.push(val.value[0]);
              sumOfValue.push(val.value[1]);
              sumOfValue.push(val.value[2]);
              sumOfValue.push(val.value[3]);
              // sumOfValue.push(val.value[4]);
              //  sumOfValue.push(val.value[5]);
              locations.push(element['location_uuid']);
            }

            child.value.push(val);
          } else {
            if (element[totalDefinition] || element[totalDefinition] === 0) {
              val.value[0] = element[totalDefinition];
              sumOfValue.push(val.value[0]);
              locations.push(element['location_uuid']);
            }
        ...
        if (this.sectionIndicatorsValues.length > 0) {
