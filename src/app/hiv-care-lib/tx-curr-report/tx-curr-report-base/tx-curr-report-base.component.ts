...
@Component({
  ...
  templateUrl: './tx-curr-report-base.component.html',
  styleUrls: ['./tx-curr-report-base.component.css']
})
export class TxCurrReportBaseComponent implements OnInit {
  @Output()
  public params: any;
  public indicators: string;
  public selectedIndicators = [];
  public txcurrReportSummaryData: any = [];
  public columnDefs: any = [];
  public reportName = 'TX_CURR Report';
  ...
  constructor(
    ...
    public txnewReport: TxCurrResourceService
  ) {
  ...
  public generateReport(): any {
    ...
    this.txcurrReportSummaryData = [];
    this.getTxCurrReport(this.params);
  }
  ...
  public getTxCurrReport(params: any) {
    this.isLoading = true;
    this.txnewReport.getTxCurrReport(params).subscribe((data) => {
      if (data.error) {
        ...
        this.txcurrReportSummaryData = data.result;
  ...
  public calculateTotalSummary() {
    const totalsRow = [];
    if (this.txcurrReportSummaryData.length > 0) {
      const totalObj = {
        location: 'Totals'
      };
      _.each(this.txcurrReportSummaryData, (row) => {
