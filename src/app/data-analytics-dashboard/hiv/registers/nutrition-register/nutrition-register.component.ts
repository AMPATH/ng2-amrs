...
export class NutritionRegisterComponent implements OnInit {
  ...
  public nutritionSummaryData: any = [];
  ...
  public generateReport(): any {
    ...
    this.getNutritionData(this.params);
  }
  ...
  public getNutritionData(params: any) {
    this.isLoading = true;
    this.register.getNutritionRegister(params).subscribe((data) => {
      if (data.error) {
  ...
  public calculateTotalSummary() {
    const totalsRow = [];
    if (this.nutritionSummaryData.length > 0) {
      const totalObj = {
        location: 'Totals'
      };
      _.each(this.nutritionSummaryData, (row) => {
