...
describe('OncologyReportPdfViewComponent', () => {
  ...
  beforeEach(() => {
    ...
    oncologyReportPdfService.generatePdf.and.returnValue(of(mockPdfResult));
  ...
  it('should generate a PDF document when passed the correct parameters', () => {
    ...
    expect(oncologyReportPdfService.generatePdf).toHaveBeenCalledTimes(1);
  ...
  it('should be able to navigate to either the previous page or the next page of the generated PDF document', () => {
    fixture.detectChanges();
    component.generatePdf();
    expect(oncologyReportPdfService.generatePdf).toHaveBeenCalledTimes(2);
  ...
  it('should have the download PDF button enabled only when a PDF document is available', () => {
    ...
    const downloadBtn = nativeEl.querySelector('.btn-primary');
    // download button disabled
    expect(downloadBtn.outerHTML).toContain('disabled');
    component.generatePdf();
    fixture.detectChanges();
    // download button enabled
    expect(downloadBtn.outerHTML).not.toContain('disabled');
    expect(oncologyReportPdfService.generatePdf).toHaveBeenCalledTimes(3);
  });
  ...
  it('should download the PDF document when the download button is clicked', () => {
    ...
    expect(oncologyReportPdfService.generatePdf).toHaveBeenCalledTimes(4);
  });
});
