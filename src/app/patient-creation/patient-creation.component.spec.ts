...
const locationResourceServiceStub = {
  ...
  getAdministrativeUnits: () => of(testAdministrativeUnits)
};
...
describe('Component: Patient Creation Unit Tests', () => {
  ...
  describe('Form actions ', () => {
    ...
    it('should submit the form when the save button is clicked after filling the form', fakeAsync(() => {
      ...
      /*
       * With demographics done, assert that fields from the Identifiers
      ...
      // Set values for county, subcounty and ward
      const countySelect: HTMLSelectElement = nativeElement.querySelector(
        'select#address1'
      );
      const subcountySelect: HTMLSelectElement = nativeElement.querySelector(
        'select#address2'
      );
      ...
      subcountySelect.value = subcountySelect.options[0].value;
    ...
    it('clicking the reset button resets the demographics section form fields', fakeAsync(() => {
      firstNameInput.value = 'Test';
      firstNameInput.dispatchEvent(new Event('input'));
