...
module.exports = function () {
    ...
    service.reportQueryServer = function (queryPartsArray, callback) {
        var multiquery = "";
        var multuvalues = [];
        var result = {};
        ...
        getServerConnection(function (err, connection) {
            ...
            connection.query(multiquery, multuvalues, function (err, rows, fields) {
                if (err) {
                    errorHandler('ExecuteMultiReport: Error querying server', err);
                    result.errorMessage = "Error querying server";
                    result.error = err;
                    result.sql = multiquery;
                    result.sqlParams = multuvalues;
                }
                else {
                    // result.startIndex = queryParts.offset || queryOffset;
                    result.size = rows.length;
