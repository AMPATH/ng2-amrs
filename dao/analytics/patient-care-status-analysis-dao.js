    ...
    return {
        getPatientCareStatusAggregatesQuery: function (queryParams, callback) {
            ...
            if (queryParams.analysis === 'cohortAnalysis') {
                ...
                column = "date_format(to_month, '%m/%Y') as reporting_month," +
            ...
            var sql =
                ...
                "from ( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, whereClause) + " ) `t1` " +


                "left outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, whereClause) + " ) t2 " +
                ...
                "right outer join( " + this.getPatientCareStatusInternalQuery(queryParams, indicators, whereClause) + " ) t2 " +
                " on t1.person_id = t2.person_id and t1.endDate = date_sub(DATE_FORMAT(t2.endDate, '%Y-%m-01'), interval 1 day) " +
        ...
        getPatientCareStatusPatientListQuery: function (queryParams, callback) {
            ...
            if (queryParams.analysis === 'cohortAnalysis') {
                ...
                where = " and date(to_month)=date('" + queryParams.endDate + "') ";
            }
            ...
            var sql =
                    ...
                    where +
                    ") t1 " +
                    " INNER JOIN amrs.person_name `person_name` ON (t1.p1 = person_name.person_id) " +
                    "LEFT OUTER JOIN amrs.patient_identifier `id` ON (t1.p1 = id.patient_id) " +
                    "INNER JOIN amrs.person `person` ON (t1.p1 = person.person_id) "
                    + ' group by t1.p1 '
        ...
        getPatientCareStatusInternalQuery: function (queryParams, indicators, whereClause) {
            var where = queryParams.analysis === 'cohortAnalysis' ? "t2.endDate = date('" + queryParams.endDate + "')" :
