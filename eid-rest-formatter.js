...
module.exports=function(){
  ...
  function convertViralLoadPayloadToRestConsumableObs(viralLoad,patientUuId){
    ...
    if(viralLoad['orderNumber'] in viralLoad){
      getOrderByOrderNumber(viralLoad['orderNumber'])
      .then(function(response){
        body.order=response.uuid;
        resolve(body);
      })
  ...
  function convertCD4PayloadTORestConsumableObs(CD4payload,patientUuId){
    ...
    var body={
      ...
      if(CD4payload['orderNumber'] in CD4payload){
        getOrderByOrderNumber(CD4payload['orderNumber'])
        .then(function(response){
          body.order=response.uuid;
          resolve(body)
        })
  ...
  function convertDNAPCRPayloadTORestConsumableObs(DNAPCRPayload,patientUuId){
    ...
    var date=moment(DNAPCRPayload.DateCollected).format();
      ...
      if(DNAPCRPayload['orderNumber'] in DNAPCRPayload){
        getOrderByOrderNumber(DNAPCRPayload['orderNumber'])
        .then(function(response){
          body.order=response.uuid;
          resolve(body)
        })
