...
module.exports = function () {
  ...
  function convertCdmConceptIdToName (indicators, queryResults, requestIndicators) {
    return new Promise(function (resolve, reject) {
      concept_name_dao.getConceptNames().then(function (tests) {
        _.each(indicators, function (indicator) {
          _.each(queryResults.result, function (row) {
            row[indicator + '_name'] = helpers.getCDMNames(tests, row[indicator]);
          });
        });
  ...
  function findChanges (indicators, queryResults, requestIndicators) {
    ...
    _.each(queryResults.result, function (row) {
      ...
      var previous = row.previous_regimen ? row.previous_regimen.split('##').sort() : null;
  ...
  function cdmMedicationChange (results) {
    ...
    _.each(med_results, function (row, index) {
      var row_promise = new Promise(function (resolve, reject) {
        var current  = row.prescriptions ? row.prescriptions.split('##').sort() : null;
        var previous = '';
        if (index > 0) {
          var previousRow = med_results[index - 1];
          previous        = previousRow.prescriptions ? previousRow.prescriptions.split('##').sort(): null;          
        }
        var diff =_.difference(previous,current);
        if(diff.length>0){            
          if (row.prescriptions) {
            addCdmMedicationChangeStatus(row).then(function (presc) {
              resolve({prescriptions: presc, encounter_datetime: row.encounter_datetime});
            }).catch(function (err) {
            });       
          } else {
  ...
  function addCdmMedicationChangeStatus (dataRow) {
    ...
    return new Promise(function (resolve, reject) {
      concept_name_dao.getConceptNames().then(function (concepts) {
        _.each(prescriptions, function (row) {
          ...
          _.each(prescription_details, function (details) {
            ...
            if (hasStatus || hasHtnStatus) { 
              ...
              result['medication_plan'] = helpers.getConceptNames(concepts,status) ? helpers.getConceptNames(concepts,status) : status;
            ...
            if (meds || htnMeds) {
              ...
              result['meds'] = helpers.getConceptNames(concepts, med) ? helpers.getConceptNames(concepts, med) : med;
            }
            ...
            if (drug_frequency) {
              var drugFrequency   = details.split('=')[1].replace('!!', '').trim();
              result['frequency'] = helpers.getConceptNames(concepts, drugFrequency) ? helpers.getConceptNames(concepts, drugFrequency) : drugFrequency;
            }
          });
          rows.push(result);
        });
