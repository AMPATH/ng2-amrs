...
export class CD4Adapter extends LabAdapter {
  ...
  mapResult(result) {
    const cd4PanelHasValidData = this.payloadFormatter.cd4PanelHasValidData(result);
    ...
    if (cd4PanelHasErrors) {
      return this.payloadFormatter.convertCD4ExceptionTORestConsumableObs(result, this.patientUuid);
    }
  }
}