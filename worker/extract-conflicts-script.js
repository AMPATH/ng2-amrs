                ...
                for (var i = 0; i < lineArray.length; i++) {
                    ...
                    if (conflict.obs.length > 0) {
                        for (var j = 0; j < conflict.obs.length; j++) {
                            ...
                            toDelete.obsDatetime = moment(obs.obsDatetime).format('DD-MMM-YY');
                            toDelete.uuid = obs.uuid;
                            toDelete.display = obs.display;
                            toDelete.value = obs.value && obs.value.uuid ? obs.value.uuid : obs.value;
                            toDelete.patientUuid = obs.person.uuid;
                            toDelete.encounterUuid = obs.encounter && obs.encounter.uuid ? obs.encounter.uuid : '';
