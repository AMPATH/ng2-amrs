...
export class MonthlyScheduleService {
        ...
        return new Promise(function (resolve, reject) {
            Promise.join(self.getAttended(reportParams), self.getScheduled(reportParams), self.getHasNotReturned(reportParams),
                (attended, scheduled, hasNotReturned) => {
                    let attendedResponse = self.buildAttendedResponse(attended.results.results);
                    let scheduledResponse = self.buildScheduledResponse(scheduled.results.results);
                    let hasNotReturnedResponse = self.buildHasnoReturnedResponse(hasNotReturned.results.results);
    ...
    getScheduled(reportParams) {
        let report = new BaseMysqlReport('dailyAppointmentsAggregate', reportParams);
        return report.generateReport();
    }

    getHasNotReturned(reportParams) {
        let report = new BaseMysqlReport('dailyHasNotReturnedAggregate', reportParams);
        return report.generateReport();
    }
