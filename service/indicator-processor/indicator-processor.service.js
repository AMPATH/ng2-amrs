...
module.exports = function () {
    ...
    function disaggregateDynamicIndicators(report, indicatorsSchema, request) {
        ...
        _.each(report.indicators, function (reportIndicator) {
            if (reportIndicator.canBeDisaggregated && reportIndicator.canBeDisaggregated === true) {
                                ...
                                case 'ageRangeFilter':
                                    ...
                                    if (params) {
                                        ...
                                        for (var i = params[0]; i < params[1]; i++) {
                                            ...
                                            var expression = s.replaceAll(filterOption.expression, '@min', i);
                                            expression = s.replaceAll(expression, '@max', i + 1);
                                            results.push(generateAggregateColumn(indicator, reportIndicator, name, expression, request));
                                        }
                                    }
                                    break;
                                case 'ageGroupFilter':
                                        ...
                                        _.each(filterOption.options, function (option) {
                                            _.each(params, function (param) {
                                                if (option.value === param) {
                                                    var name = indicator.name + '_age_' + toSnakeCase(option.value);
                                                    var expression = s.replaceAll(filterOption.expression, '@min', option.queryParam[0]);
                                                    expression = s.replaceAll(expression, '@max', option.queryParam[1]);
                                                    results.push(generateAggregateColumn(indicator, reportIndicator, name, expression, request));
                                                }
                                            });
                                        });
                                ...
                                case 'genderFilter':
                                    ...
                                    if (params) {
                                        ...
                                        _.each(filterOption.options, function (option) {
                                            _.each(params, function (param) {
                                                if (option.value === param) {
                                                    var name = indicator.name + '_' + toSnakeCase(option.value);
                                                    var expression = s.replaceAll(filterOption.expression, '@gender', option.queryParam);
                                                    results.push(generateAggregateColumn(indicator, reportIndicator, name, expression, request));
                                                }
                                            });
                                    ...
                                    if (params) {
                                        ...
                                        _.each(filterOption.options, function (option) {
                                            _.each(params, function (param) {
                                                if (option.value === param) {
                                                    var name = indicator.name + '_' + toSnakeCase(option.value);
                                                    var expression = s.replaceAll(filterOption.expression, '@paramBound', option.queryParam);
                                                    results.push(generateAggregateColumn(indicator, reportIndicator, name, expression, request));
                                                }
                                            });
    ...
    function generateAggregateColumn(indicator, reportIndicator, name, expression, requestParam) {
