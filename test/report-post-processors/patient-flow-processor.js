...
describe('PATIENT FLOW PROCESSOR TESTS', function () {
    ...
    it('should inject patient flow samples',
        function () {
            expect(patientFlowSamples).to.be.defined;
        });

    it('should inject patient flow module',
        function () {
            expect(patientFlowProcessor).to.be.defined;
        });

    it('should group results by visit id when groupResultsByVisitId',
        function () {
            var unGroupedResults = patientFlowSamples.getPatientFlowSample();
            var groupedResults = patientFlowProcessor.groupResultsByVisitId(unGroupedResults.result);
            ...
            expect(groupedResults[0].seen_by_clinician).to.equal("2016-06-15T05:35:44.000Z");
            expect(groupedResults[1].seen_by_clinician).to.equal("2016-06-15T05:40:51.000Z");
            ...
            expect(groupedResults[0].completed_visit).to.equal("2016-06-15T05:44:25.000Z");
            expect(groupedResults[1].completed_visit).to.equal("2016-06-15T05:56:59.000Z");
            expect(groupedResults[2].completed_visit).to.equal("2016-06-15T05:48:14.000Z");

            //CHECK 9: calculates time_to_be_seen_by_clinician
            expect(groupedResults[0].time_to_be_seen_by_clinician !== undefined).to.be.true;

            //CHECK 10: calculates time_to_complete_visit
            expect(groupedResults[0].time_to_complete_visit !== undefined).to.be.true;
        });

    it('should calculate time span in minutes when _getTimeSpanInMinutes',
        function () {

            var difference =
                patientFlowProcessor._getTimeSpanInMinutes("2016-06-15T05:16:34.000Z", "2016-06-15T05:46:30.000Z");
            expect(difference).to.equal(30);
        });

    it('should calculate time to be seen by clinician correctly when _handleTimeToBeSeenByClinician',
        function () {
    ...
    it('should calculate time to complete visit correctly when _handleTimeToBeSeenByClinician',
        function () {
