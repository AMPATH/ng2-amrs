...
describe('openmrsLocationAuthorizer', function () {
    ...
    it('makes sure that openmrsLocationAuthorizer can be enabled only for secured routes', function (done) {
        ...
        server.register(plugin, {}, function (err) {
            server.start(function (err) {
                ...
                expect(err).to.match(/openmrsLocationAuthorizer can be enabled only for secured route/);
    ...
    it('should allow openmrsLocationAuthorizer for routes secured globally with authentication and blacklisting routes' +
        ' to require authorization', function (done) {
        var server = new Hapi.Server(0);
        server.connection({
            routes: {
                plugins: {
                    openmrsLocationAuthorizer: {locationParameter: [{type: 'params', name: 'uuid'}]}
                }
            }
        });
        server.auth.scheme('custom', internals.authSchema);
    ...
    it('should not error with global authentication set, blacklisting routes to require authorization, and disabling ' +
        'authentication and authorization for a specific route', function (done) {
...
internals.authSchema = function () {
    ...
    var scheme = {
        authenticate: function (request, reply) {
            return reply({username: "asafdav", locationParameter: [{type: 'params', name: 'uuid'}]});
        },
