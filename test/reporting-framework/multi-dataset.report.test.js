...
describe('MultiDatasetReport:', () => {
    ...
    it('should generate reports', (done) => {
        ...
        let fetchReportSchemaStub = sinon.stub(report, '_fetchAndInitReports')
            .returns(Promise.resolve(reports));

        let executeReportHandlersStub = sinon.stub(report, 'executeReportHandlers')
        ...
        report.generateReport(additionalParams).then((generated) => {
            ...
            expect(executeReportHandlersStub.calledWithExactly(report.reportHandlers, additionalParams)).to.be.true;
    ...
    it('should execute report handlers', (done) => {
        ...
        let runSingleReportStub = sinon.stub(report, 'runSingleReport')
            .returns(Promise.resolve(sampleResult));

        report.executeReportHandlers(reportHandlers, additionalParams)
            .then((results) => {
                expect(runSingleReportStub.getCalls()[0].calledWithExactly(reportHandlers[0], additionalParams));
