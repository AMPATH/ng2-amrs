...
module.exports=function(){
    ...
    return {
        ...
        getHivSummaryIndicators:function getHivSummaryIndicators(request,callback){
            ...
            var arrayResults=[];
            /**
            The function  defination  of reportFactory.singleReportToSql
            will not  change  because of  legacy usage. when  all legacy
            implementation  have  been  updated it  should  be  revised
            **/
            var  singleReportQueryParams=reportFactory.singleReportToSql(requestParams,null);
          console.log(singleReportQueryParams,'<<<<<<<Single report Query Params may have  multiple depending  of report type')
 var multiQueryPartsArray=db.reportMultiQueryServer(singleReportQueryParams);
 console.log(multiQueryPartsArray,'The Multi query query parameters')
  db.ExecuteMultiReport(multiQueryPartsArray[0],multiQueryPartsArray[1],callback)
