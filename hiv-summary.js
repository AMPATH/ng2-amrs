...
function dateDiff(d1,d2,interval) {
    ...
    if(interval === undefined || interval.toLowerCase() === "day") return (d1 - d2)/(24*60*60*1000);
    if(interval.toLowerCase() === "week") return (d1 - d2)/(7*24*60*60*1000);
    if(interval.toLowerCase() === "year") return (d1 - d2)/(52*7*24*60*60*1000);
}
...
var hivSummary = function(uuid) {
    ...
    this.getData = function() {
        var q = "select * from etl.flat_obs where uuid='" + uuid + "' order by encounter_datetime";
        queryServer(query,null,function(data) {that.data = data;});
    }
    ...
    this.functions.getFirstEvidencePatientPregnant = function(curRow,prevCalc) {
        ...
        if (value === undefined && ([32, 33, 44, 10].indexOf(curEncounter.encounter_type) !== -1 || /\b1279=|\b5596=/.test(curRow.obs)))
